...
class MyEnv(gym.Env):
	def __init__(self):
		...
		self.co2_per_step      = []   # emissioni CO2
		self.nox_per_step      = []   # emissioni NOx
		self.pmx_per_step      = []   # emissioni PMx
		self.fuel_per_step     = []   # consumo carburante
	...
	def step(self, action):
		...
		#print(f"DEBUG: CO2 totale episodio = {self.episode_co2}, waiting medio = {np.mean(self.episode_waiting_times) if self.episode_waiting_times else 0}")
		

		# modifica delle variabili da riportare nel CSV
		...
		'''if self.current_step % 100 == 0: 
			print(f"Step {self.current_step}: CO2_step = {co2_step:.2f} mg")'''
		...
		# Pedoni in attesa (media o somma? qui media)
	...
	def _get_reward(self):
		...
		#print((ret + balance_penalty)*20, "---", long_green_penalty, "---", safety_signal)
		#if(long_green_penalty != 0): print(long_green_penalty)
		return (ret + balance_penalty)*20 + long_green_penalty # + safety_signal
